...
void ecrire_Liste_Bezier(Liste_Bezier2 L)
{
	...
	{	
		...
		printf(" ((%f,%f), (%f,%f), (%f,%f))\n", P.C0.x, P.C0.y, P.C1.x, P.C1.y, P.C2.x, P.C2.y);
	} 
	printf("]\n");
...
Liste_Liste_Bezier2 ajouter_element_liste_liste_Bezier2(Liste_Liste_Bezier2 L, Liste_Bezier2 e)
{
	...
	el = creer_element_liste_liste_Bezier2(e);
	if (L.taille == 0)
	{
		/* premier element de la liste */
		L.first = L.last = el;
...
{
	Tableau_Liste_Bezier2 T;
	
	/* taille de T = taille de L */
	T.taille = L.taille;
	
	/* allocation dynamique du tableau de Bezier2 */
	T.tab = malloc(sizeof(Liste_Bezier2) * T.taille);
	if (T.tab == NULL)
	{
		/* allocation impossible : arret du programme avec un message */
		fprintf(stderr, "sequence_Bezier2s_liste_vers_tableau : ");
	...
	/* remplir le tableau de Bezier2s T en parcourant la liste L */
	int k = 0; /* indice de l'element dans T.tab */
	Cellule_Liste_Liste_Bezier2 *el = L.first; /* Bezier2eur sur l'element dans L */
	while (el) 
	{
		T.tab[k] = el->data;
		k++; /* incrementer k */
